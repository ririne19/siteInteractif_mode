...
if (($_FILES["limage"]["size"] > $file_min_size) && ($_FILES["limage"]["size"] < $file_max_size)) :
    ...
    $url_image = "image_site/defile/" . $name_file;
    ...
    //ajout de la photo lié a la defile
    $requete_preparee = $bdd->prepare('INSERT into represente (id_photo, id_defile) VALUES(:id_photo, :id_defile)');
    $requete_preparee->bindValue(':id_photo', $id_photo, PDO::PARAM_STR);
    $requete_preparee->bindValue(':id_defile', $_POST["id_defile"], PDO::PARAM_STR);
    $res = $requete_preparee->execute();
